...
import { blogStore, BlogCategory, BlogPostItem } from '../../lib/blogStore';
...
export default function CmsCategories({
  ...
  const [tags, setTags] = useState<BlogTag[]>([
    { id: 'tag-1', name: 'Bereavement', slug: 'bereavement', postCount: 5 },
    { id: 'tag-2', name: 'Woodland Burial', slug: 'woodland-burial', postCount: 4 },
    { id: 'tag-3', name: 'Pre-Planning', slug: 'pre-planning', postCount: 8 },
    { id: 'tag-4', name: 'Veterans Honors', slug: 'veterans-honors', postCount: 3 },
    { id: 'tag-5', name: 'Grief Support', slug: 'grief-support', postCount: 6 },
    { id: 'tag-6', name: 'Eulogy Guidance', slug: 'eulogy-guidance', postCount: 4 },
    { id: 'tag-7', name: 'Urns & Memorials', slug: 'urns-memorials', postCount: 3 }
  ]);
  ...
  const handleSaveCategory = (e: React.FormEvent) => {
    ...
    if (editingCatId) {
      const updatedCats = categories.map(c => {
        if (c.id === editingCatId) {
          ...
          if (oldName !== catFormData.name.trim()) {
            const updatedPosts = posts.map(p => p.category === oldName ? { ...p, category: catFormData.name.trim() } : p);
      ...
      onShowToast(`Category "${catFormData.name}" updated successfully!`);
    } else {
      ...
      onShowToast(`Category "${catFormData.name}" created!`);
    }
  ...
  const handleConfirmDelete = () => {
    ...
    if (deleteTarget.type === 'category') {
      ...
      setTags(filtered);
      onShowToast(`Tag "#${tag.name}" deleted.`);
    }
  ...
  const handleSaveTag = (e: React.FormEvent) => {
    ...
    if (editingTagId) {
      ...
      setTags(updatedTags);
      onShowToast(`Tag "#${cleanName}" updated successfully!`);
    } else {
      ...
      setTags([...tags, newTag]);
      onShowToast(`Tag "#${cleanName}" created!`);
    }
  ...
  const handleMergeTags = (e: React.FormEvent) => {
    ...
    if (!sourceTagId || !targetTagId || sourceTagId === targetTagId) {
      onShowToast('Please select two distinct tags to merge.');
      return;
    }
    ...
    onShowToast(`Merged "#${sourceTag.name}" into "#${targetTag.name}". All articles updated.`);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* 1. HEADER BANNER DIRECTLY FROM ATTACHED DESIGN */}
      <div className="bg-gradient-to-r from-[#411548] via-[#2f0d34] to-[#411548] text-white p-6 md:p-8 rounded-2xl shadow-lg border border-[#C5A059]/30 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <FolderTree className="w-6 h-6 text-[#C5A059] shrink-0" />
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-black uppercase transition-all cursor-pointer ${
                activeTab === 'categories'
                  ? 'bg-[#C5A059] text-[#411548] shadow-md'
        ...
        <p className="text-xs md:text-sm text-gray-200 font-medium leading-relaxed max-w-4xl">
          Create, edit, and manage funeral guidance categories and memorial tags to organize publication content across your platform.
        </p>
        ...
        <div className="pt-2 flex flex-wrap items-center gap-2 sm:gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#C5A059] hover:bg-[#b08b43] text-[#411548] rounded-xl text-xs font-black uppercase shadow-md transition-all cursor-pointer"
          >
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#C5A059] transition-all"
          />
          {searchQuery && (
            <button 
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#C5A059]"
            >
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
          <select
            ...
            className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 outline-none focus:border-[#C5A059] cursor-pointer"
          >
          ...
          {activeTab === 'categories' ? (
            <button
              onClick={handleOpenNewCategory}
              className="flex items-center gap-2 px-4 py-2 bg-[#411548] hover:bg-[#C5A059] text-white hover:text-[#411548] border border-[#C5A059]/50 rounded-xl text-xs font-black uppercase shadow-xs transition-all cursor-pointer group"
            >
              <Plus className="w-4 h-4 text-[#C5A059] group-hover:text-[#411548] transition-colors" />
              <span>New Category</span>
            </button>
          ) : (
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowMergeModal(true)}
                className="flex items-center gap-2 px-3 py-2 bg-purple-50 hover:bg-[#411548] text-[#411548] hover:text-white border border-[#C5A059]/40 rounded-xl text-xs font-black uppercase transition-all cursor-pointer"
              >
                <Layers className="w-4 h-4 text-[#C5A059]" />
                <span>Merge Tags</span>
              </button>
        ...
        <div className="space-y-4">
          {filteredCategories.length === 0 ? (
            <div className="bg-white p-12 rounded-2xl border border-gray-200 text-center space-y-4">
              <div className="w-12 h-12 bg-amber-50 rounded-full flex items-center justify-center mx-auto text-[#C5A059] border border-[#C5A059]/30">
                <FolderTree className="w-6 h-6" />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm font-black text-gray-800 uppercase">No Categories Found</h3>
                <p className="text-xs text-gray-500 max-w-sm mx-auto">
                  {searchQuery ? `No category matching "${searchQuery}"` : 'Get started by creating your first guidance category.'}
                </p>
              </div>
              <button
                onClick={handleOpenNewCategory}
                className="px-4 py-2 bg-[#411548] hover:bg-[#C5A059] text-white hover:text-[#411548] border border-[#C5A059]/50 text-xs font-bold rounded-xl cursor-pointer transition-all"
              >
                + Create Category
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCategories.map((cat) => (
                <div 
                  key={cat.id} 
                  className="bg-white rounded-2xl border-t-2 border-t-[#C5A059] border-x border-b border-gray-200 p-5 shadow-xs hover:border-[#C5A059] hover:shadow-md transition-all flex flex-col justify-between space-y-4 group"
                >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span 
                          ...
                          style={{ backgroundColor: cat.color || '#C5A059' }} 
                        />
                        <h3 className="font-black text-sm text-[#411548] uppercase tracking-tight group-hover:text-[#C5A059] transition-colors">
                          {cat.name}
                        </h3>
                      </div>
                      <span className="px-2.5 py-1 bg-amber-50 text-[#7a600d] border border-[#C5A059]/30 rounded-full text-[10px] font-black shrink-0">
                  ...
                  <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                    <button
                      ...
                      className="text-[11px] font-black text-[#411548] hover:text-[#C5A059] flex items-center gap-1 cursor-pointer transition-colors"
                    >
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        title="Edit Category"
                        className="p-1.5 bg-gray-100 hover:bg-[#411548] hover:text-white rounded-lg text-gray-600 transition-all cursor-pointer"
                      >
      ...
      {activeTab === 'tags' && (
        <div className="bg-white p-6 rounded-2xl border-t-2 border-t-[#C5A059] border-x border-b border-gray-200 shadow-xs space-y-6">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <div>
              <h3 className="font-black text-sm text-[#411548] uppercase flex items-center gap-2">
                <TagIcon className="w-4 h-4 text-[#C5A059]" />
                All Memorial Guidance Tags ({filteredTags.length})
              </h3>
              <p className="text-xs text-gray-500 font-medium">
                Click any tag to edit or delete. Tags organize articles across service options, grief support themes, and heritage planning.
              </p>
            </div>
          </div>
          ...
          {filteredTags.length === 0 ? (
            <div className="text-center py-12 space-y-3">
              <TagIcon className="w-8 h-8 text-[#C5A059]/50 mx-auto" />
            ...
            <div className="flex flex-wrap gap-2.5">
              {filteredTags.map((tag) => (
                <div 
                  key={tag.id}
                  className="group inline-flex items-center gap-2 px-3.5 py-2 bg-amber-50/40 hover:bg-amber-50 border border-amber-200/80 hover:border-[#C5A059] rounded-xl transition-all"
                >
                  <span className="text-xs font-black text-[#411548]">
                    <span className="text-[#C5A059] mr-0.5">#</span>{tag.name}
                  </span>
                  
                  <span className="px-2 py-0.5 bg-white border border-[#C5A059]/30 group-hover:border-[#C5A059] rounded-full text-[10px] font-black text-[#7a600d]">
                    {tag.postCount}
                  </span>
                  ...
                  <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity pl-1 border-l border-amber-200">
                    <button
                      ...
                      title="Edit Tag"
                      className="p-1 hover:text-[#411548] cursor-pointer"
                    >
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border-t-4 border-t-[#C5A059] border-x border-b border-gray-200 overflow-hidden space-y-0">
            <div className="bg-[#411548] text-white p-5 flex items-center justify-between border-b border-[#C5A059]/30">
              <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="text-white/80 hover:text-[#C5A059] p-1 cursor-pointer transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveCategory} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-black text-[#411548] uppercase mb-1">
                  Category Name *
                </label>
                <input
                  type="text"
                  ...
                  placeholder="e.g. Woodland Eco-Burial & Green Funerals"
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#C5A059]"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-mono text-[#C5A059] font-bold">
                    /category/
                  </span>
                  <input
                    type="text"
                    ...
                    placeholder="woodland-eco-burial"
                    className="w-full pl-24 pr-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono outline-none focus:bg-white focus:border-[#C5A059]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Brief description of the memorial articles contained in this category..."
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#C5A059]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 outline-none focus:border-[#C5A059] cursor-pointer"
                  >
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-[#411548] hover:bg-[#C5A059] border border-[#C5A059]/50 text-white hover:text-[#411548] rounded-xl text-xs font-black uppercase shadow-xs cursor-pointer transition-all"
                >
      ...
      {showTagModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border-t-4 border-t-[#C5A059] border-x border-b border-gray-200 overflow-hidden">
            <div className="bg-[#411548] text-white p-5 flex items-center justify-between border-b border-[#C5A059]/30">
              <div className="flex items-center gap-2">
                <TagIcon className="w-5 h-5 text-[#C5A059]" />
                <h3 className="font-black text-sm uppercase">
            ...
            <form onSubmit={handleSaveTag} className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-black text-[#C5A059]">
                    #
                  </span>
                  <input
                    type="text"
                    ...
                    placeholder="EulogyIdeas"
                    className="w-full pl-8 pr-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none focus:bg-white focus:border-[#C5A059]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="eulogy-ideas"
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono outline-none focus:bg-white focus:border-[#C5A059]"
                />
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-[#411548] hover:bg-[#C5A059] border border-[#C5A059]/50 text-white hover:text-[#411548] rounded-xl text-xs font-black uppercase cursor-pointer transition-all"
                >
            ...
            <form onSubmit={handleMergeTags} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none focus:border-[#C5A059] cursor-pointer"
                >
                  <option value="">-- Select tag to merge from --</option>
                  {tags.map(t => (
                    <option key={t.id} value={t.id}>#{t.name} ({t.postCount || 0} posts)</option>
                  ))}
                </select>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-[#C5A059] hover:bg-[#411548] text-[#411548] hover:text-white border border-[#C5A059] font-black rounded-xl text-xs uppercase cursor-pointer transition-all"
                >
                  Execute Merge
                </button>
              </div>
